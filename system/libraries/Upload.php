...
class CI_Upload {
	...
	// --------------------------------------------------------------------
	function do_upload2($field2 = 'userfile')
	{
		// Is $_FILES[$field] set? If not, no reason to continue.
		if ( ! isset($_FILES[$field2]))
		{
			$this->set_error('upload_no_file_selected');
			return FALSE;
		}
		...
		// Was the file able to be uploaded? If not, determine the reason why.
		if ( ! is_uploaded_file($_FILES[$field2]['tmp_name']))
		{
			$error = ( ! isset($_FILES[$field2]['error'])) ? 4 : $_FILES[$field2]['error'];
		...
		// Set the uploaded data as class variables
		$this->file_temp = $_FILES[$field2]['tmp_name'];		
		$this->file_name = $this->_prep_filename($_FILES[$field2]['name']);
		$this->file_size = $_FILES[$field2]['size'];		
		$this->file_type = preg_replace("/^(.+?);.*$/", "\\1", $_FILES[$field2]['type']);
		$this->file_type = strtolower($this->file_type);
		$this->file_ext	 = $this->get_extension($_FILES[$field2]['name']);
	...
	//buat upload
	function fileupload2($field2, $path, $types, $max_size = 0, $max_width=0, $max_height=0)
	{
		$config['upload_path'] = $path;
		$config['allowed_types'] = $types;
		$config['max_size']	= $max_size;
